...
const account = {
    ...
    add: function(add_data, callback){
        return db.query(
            'insert into account (accountId,balance,personId,cardId,creditLimit) values(?,?,?,?,?)',
            [add_data.accountId, add_data.balance, add_data.personId, add_data.cardId, add_data.creditLimit],
            callback
        );
    },
    ...
    getByCardId: function(id, callback){
        return db.query('select * from account where cardId=?', [id], callback);
    },
    updateBalance: function(id, update_data, callback){
        return db.query(

...
const student = {
    ...
    add: function(add_data, callback){
        bcrypt.hash(add_data.password, saltRounds, function(err, hashPass){
            return db.query(
                'insert into student (id_student,fname,lname,password) values(?,?,?,?)',
                [add_data.id_student, add_data.fname, add_data.lname, hashPass],
                callback
            );
        });
    },
    ...
    update: function(id, update_data, callback){
        bcrypt.hash(update_data.password, saltRounds, function(err, hashPass){
            return db.query(
                'update student set fname=?, lname=?, password=? where id_student=?',
                [update_data.fname, update_data.lname, hashPass, id],
                callback
            );
        });
    },
    checkPassword: function(id_student,callback){
        return db.query(
            'select password from student where id_student=?',
            [id_student],
            callback
        )
    }
